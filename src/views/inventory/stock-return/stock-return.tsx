import {
  ...
  apiGetListStockReturn,
} from "@/api/endpoints/stock";
import { BaseButton } from "@/components/base/button";
import { useCallback, useEffect, useState, type FormEvent } from "react";
import { StockReturnForm } from "./stock-return-form";
import type { Stocks, StocksPayload } from "@/types/stock";
import BaseAlert from "@/components/base/alert";
import { StockReturnTable } from "./stock-return-table";
...
const StockReturnPage = () => {
  ...
  const submitStockIn = async () => {
    ...
    const { error } = await apiCreateStockReturn({
      ...form,
      qty: Number(form.qty),
    });
    if (!error) {
  ...
  const loadStock = useCallback(
    async (keyword?: string | null) => {
      ...
      const { data, error } = await apiGetListStockReturn({
        ...params,
        keyword,
      });
      if (!error && data.data) {
  ...
  return (
    <LayoutStock>
      ...
      {canAccess("stock_return_create") && (
        <>
          <div className="flex gap-4 items-center justify-between">
            <h1 className="font-semibold text-lg">Stock Return Management</h1>
          ...
          <StockReturnForm form={form} setForm={setForm} />
        </>
      )}
      <div className="space-y-4">
        <h1 className="font-semibold text-lg">Stock Return Table</h1>
