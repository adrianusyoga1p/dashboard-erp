...
import { apiImportProduct } from "@/api/endpoints/product";
...
export const ProductModal = ({ trigger, loadData }: ProductModalProps) => {
  ...
  const submitImport = async () => {
    if (!file) return;
    console.log(file);
    const { error } = await apiImportProduct(file);
    if (!error) {
      setSubmitStatus({
        isOpen: true,
        type: "success",
      ...
      >
        ...
        {data.length > 0 && columns.length > 0 && (
          <div className="w-full overflow-x-auto">
            <BaseTable<ProductPayload> source={data} columns={columns} />
          </div>
        )}
      </BaseModal>
    </>
